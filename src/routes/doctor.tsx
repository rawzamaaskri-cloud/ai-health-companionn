...
import { LayoutDashboard, Users, FileText, Activity, LogOut, Menu, X, Stethoscope, Calendar, Bell } from "lucide-react";
...
const NAV = [
  ...
  { to: "/doctor/appointments", label: "Rendez-vous", icon: Calendar },
  { to: "/doctor/prescriptions", label: "Ordonnances", icon: FileText },
  { to: "/doctor/monitoring", label: "Monitoring IoT", icon: Activity },
];

function DoctorLayout() {
  const { user, loading } = useAuth();
  const { role, loading: roleLoading, isDoctor } = useRole();
  ...
  useEffect(() => {
    if (!roleLoading && user && role && !isDoctor) {
      toast.error("Accès médecin requis.");
      nav({ to: "/app" });
    }
  }, [roleLoading, role, isDoctor, user, nav]);
  ...
  if (loading || roleLoading || !user || !isDoctor) {
    return (
    ...
    <div className="min-h-screen bg-gradient-soft">
      ...
      <header className="sticky top-0 z-40 border-b border-border bg-background/80 backdrop-blur-md md:hidden">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-2">
            <Logo withText />
            <span className="rounded-full bg-blue-500/10 px-2 py-0.5 text-[10px] font-bold uppercase text-blue-600">Médecin</span>
          </div>
            ...
            {NAV.map((n) => (
              <Link key={n.to} to={n.to} onClick={() => setOpen(false)}
                className={cn("flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium",
                  isActive(n.to, n.exact) ? "bg-blue-500/10 text-blue-700" : "text-muted-foreground hover:bg-muted")}>
      ...
      <div className="mx-auto flex max-w-7xl">
        ...
        <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-r border-border bg-sidebar px-4 py-6 md:flex">
          ...
          <div className="mt-2 flex items-center gap-1.5 rounded-lg bg-blue-500/10 px-2.5 py-1.5 text-xs font-semibold text-blue-600">
